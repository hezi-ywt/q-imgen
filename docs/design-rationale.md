...
## Why the Gemini client absorbs `/Users/ywt/Documents/动漫图像设计/nanobanana/`

老版本通过 `subprocess.run([python, -m, nanobanana, ...])` 派发到一个本地包,这个本地包不在 pip 管理里,q-imgen 的"可安装"性实际上是假的 —— 任何没有那个本地包的环境都跑不起来。

现在 `gemini_client.py` 从那个本地包吸收了约 120 行核心代码(HTTP + Gemini payload + 认证分支),q-imgen 彻底不依赖外部包。`/Users/ywt/Documents/动漫图像设计/nanobanana/` 保留不动,两者独立演化。
