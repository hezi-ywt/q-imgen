...
4. OpenAI Images: calls `/images/generations`, maps `--image` to `input_images`, normalizes shortcuts like `2K` to explicit pixel sizes, enforces provider size limits (max edge 3840, 16px multiples, <=3:1 aspect, 655360-8294400 pixels), handles b64/URL responses, and forwards Images-specific params only when set
